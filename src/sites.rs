use rocket::{
    http::{Status, uri::{Uri, Absolute}},
    response::{Redirect, Debug}, 
    State, form, form::Form};
use tokio::sync::Mutex;
use std::collections::BTreeMap;
use anyhow::{Error, Result, anyhow, Context};
use octocrab::{Octocrab, models::repos::{CommitAuthor, FileUpdate}};
use addr::parse_domain_name;
use uuid::Uuid;

use crate::{User, SitesMap, SiteData, ClientTokens, get_named_sites,
    get_site_content, get_deserialized_sites, GH_USER, GH_REPO, GH_SITES_PATH};
...
pub async fn add(user: User, sites_data: &State<SitesMap>, form: Form<WebsiteSignup>, 
...
pub async fn update(user: User, sites_data: &State<SitesMap>, form: Form<WebsiteSignup>, id: u32,
    gh_client: &State<Mutex<Octocrab>>, client_tokens: &State<Mutex<ClientTokens>>) 
...
pub async fn delete(user: User, sites_data: &State<SitesMap>, id: u32, 
    gh_client: &State<Mutex<Octocrab>>, client_tokens: &State<Mutex<ClientTokens>>) 
        -> Result<Redirect, (Status, Debug<Error>)> {
    let locked_gh_client = gh_client.lock().await;   
    let (mut recurse_sites, sites_sha) = get_sites_and_shas(&locked_gh_client).await
